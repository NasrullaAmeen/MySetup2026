...
- MainArch (the "MainVM") gets Internet via host NAT on whatever uplink is
  active; VMs keep talking to the host even with no Internet.
...
(802.11 client cannot do transparent L2 bridging). Never give the only Wi-Fi
...
- Work from the local console first time, or keep a fallback (hotspot / USB
  tether / hub LAN).
...
Static IPs above are preferred (predictable). If you want DHCP: dnsmasq on the
host, single server on vmbr0 only:
...
- **Option B:** home-LAN bridge for direct router addressing:
  ...
  then a second NIC on chosen VMs -> vmbr1. Keep NIC#1 on vmbr0 so the private
  network survives hub removal. (A single hub Ethernet port is not a switch -
  plug it into a router/switch.)

## 13. Physical Wi-Fi to a VM - do not
